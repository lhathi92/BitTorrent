...
class peertemp1 extends Thread
{
 int i=1;
int j=1;
public int id=0;  
   ...
   public void run()
	...
	recieveFile(s1);
		 ...
		 System.out.println("the problem is here");
     	   Logger.getLogger(peertemp1.class.getName()).log(Level.SEVERE, null, ex);
    	}
	...
	public void recieveFile(Socket s) throws IOException
	{
			...
			System.out.println("chunks in a group="+x);

      			 String fileName = clientData.readUTF();
			System.out.println("FileName="+fileName);

          			DataOutputStream output = new DataOutputStream(new FileOutputStream(new File("client1/",fileName)));
	...
	ss=new ServerSocket(14001);

		for(int p=1;p<=id;p++)
			{
			String file_name=String.valueOf(p);
			 File f = new File("client1/file_name");
				if(f.exists())
					{
	...
	public void append(int id) throws IOException
		...
		int bytesread=0;
		int mbytes;
		String op1="client1/op";
		File op=new File(op1);
		
		for(int i=1;i<=id;i++)
		{
	String fnm=String.valueOf(i);
	File f=new File("client1/"+fnm);
	if(f.exists())
	{
...
peertemp1 client = new peertemp1();
