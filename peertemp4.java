...
class peertemp4 extends Thread
{
      int i=1;
 int j=1;
public int id=0;  
 ...
 s1=  connection1(s,port);
	...
	recieveFile(s1);
		 ...
		 System.out.println("the problem is here");
	        Logger.getLogger(peertemp.class.getName()).log(Level.SEVERE, null, ex);
    }
	...
	{ 
		...
		append(id);	
        	...
        	Logger.getLogger(peertemp4.class.getName()).log(Level.SEVERE, null, ex);
    		}    
}
        }
...
ss=new ServerSocket(14004);

			for(int p=1;p<=id;p++)
			{
			String file_name=String.valueOf(p);
			 File f = new File("client4/file_name");
				if(f.exists())
					{
...
System.out.println("FileName="+fileName);
            ...
            DataOutputStream output = new DataOutputStream(new FileOutputStream(new File("client4/",fileName)));
            long size = clientData.readLong();
System.out.println("size="+size);
...
public void append(int id) throws IOException
	...
	int bytesread=0;
	int mbytes;
		String op1="client4/op";
	File op=new File(op1);
	for(int i=1;i<=id;i++)
	{
	String fnm=String.valueOf(i);
	File f=new File("client4/"+fnm);
	if(f.exists())
	{
...
peertemp4 client = new peertemp4();
