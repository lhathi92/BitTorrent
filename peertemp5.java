...
class peertemp5 extends Thread
{
 int i=1;
int j=1;
public int id=0;  
	...
	finally
	{
		...
		for(int u=1;u<=id;u++)
		{
			...
			System.out.println("run1to send error "+ie);  
		        ...
		        Logger.getLogger(peertemp5.class.getName()).log(Level.SEVERE, null, ex);
   			 }    
	}
        }
...
ss=new ServerSocket(14005);

		for(int p=1;p<=id;p++)
			{
			String file_name=String.valueOf(p);
			 File f = new File("client5/file_name");
				if(f.exists())
					{
...
System.out.println("FileName="+fileName);
 ...
 DataOutputStream output = new DataOutputStream(new FileOutputStream(new File("client5/",fileName)));
   long size = clientData.readLong();
System.out.println("size="+size);
	//int sz=(int)size;
...
public void append(int id) throws IOException
	...
	int bytesread=0;
	int mbytes;
		String op1="client5/op";
	File op=new File(op1);
	for(int i=1;i<=id;i++)
	{
	String fnm=String.valueOf(i);
	File f=new File("client5/"+fnm);
	if(f.exists())
	{
...
peertemp5 client = new peertemp5();
